...
//			if(count % 10 == 0)
		...
		*/
		...
		double maxMag = Utilities.magnitude(Math.abs(sx1) + Constants.ROBOT_WIDTH_TO_RADIUS_RATIO*Math.abs(sx2),
				Math.abs(sy1) + Constants.ROBOT_LENGTH_TO_RADIUS_RATIO*Math.abs(sx2));
		...
		else if (fast && !driveIO.getOpControl()) {
			fl.setDrive(fld);
			fr.setDrive(frd);
			bl.setDrive(bld);
			br.setDrive(brd);
...
//		prevSlow = slowButton;
		...
		//System.out.println(fl.getAngle());
	}
...
//		return Math.abs(DriveIO.getInstance().getPivots()[2].getVelocity());
	...
	private double getCurrent(){
		PDPManagerIO pdp = PDPManagerIO.getInstance();
//		System.out.println("Motor current: " + pdp.getCurrent(PortConstants.MotorFLD));
		return pdp.getCurrent(PortConstants.MotorFLD);
	}
